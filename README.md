...
Проект по созданию полной инфраструктуры приложения в Kubernetes:
от базовых лимитов и баз данных до собственного Kubernetes Operator на языке Go.
...
# (Опционально) Добавление второго узла для проверки DaemonSet
minikube node add
```
...
# Проверка квоты (почему может не хватать ресурсов на 5 реплик)
...
# Проверка расшифровки секрета
kubectl get secret app-credentials \
  -n k8s-lifecycle-prod \
...
kubectl apply -f app-stateless/

# База данных (StatefulSet + PV + StorageClass)
...
# Бэкап и автоматическая чистка (CronJob)
...
kubectl get ms -n k8s-lifecycle-prod
# (ms = mysites)

# Логи приложения
kubectl logs -l app=lifecycle-app -n k8s-lifecycle-prod

# Статус масштабирования
kubectl describe deployment marketing-site-deploy -n k8s-lifecycle-prod
```
